const getB = b => {
  ...
  const dealDamage = player => {
    turn = !turn;
    var damage = getRand(50);
    player.realHP -= damage;
    player.container.style.animation = "shake "+(effectTime/1000)+"s linear";
  ...
  getB('character-btn-kick').addEventListener('click', function() {
    if (turn) {
      ...
      alert('Наразі ходить ' + charmander.name);
    }
  });
  ...
  getB('enemy-btn-kick').addEventListener('click', function() {
    if (!turn) {
      ...
      alert('Наразі ходить ' + pikachu.name);
    }
  });
